 ...
 */
@WebServlet("/manageUpdate.fbo")
public class FreeboardManageUpdateController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FreeboardManageUpdateController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			if(result > 0) { // 성공 => 해당 게시글의 상세조회 페이지로 url 재요청 
				...
				response.sendRedirect(request.getContextPath() + "/manageDetail.fbo?bno=" + boardNo);
