 ...
 */
@WebServlet("/manageUpdate.sbo")
public class ShareboardManageUpdateController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ShareboardManageUpdateController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			// 서비스 요청
			int result = new ShareboardService().updateBoard(sb, deleteImgList,  updateImgList);
			
			// 결과값에 따른 응답 처리
			if(result > 0) { // 성공 => 해당 게시글의 상세조회 페이지로 url 재요청 
				request.getSession().setAttribute("alertMsg", "게시글이 성공적으로 수정되었습니다.");
				response.sendRedirect(request.getContextPath() + "/manageDetail.sbo?bno=" + boardNo);
			} else { // 실패 => 에러페이지 포워딩
