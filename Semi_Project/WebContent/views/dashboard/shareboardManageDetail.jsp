<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="java.util.ArrayList, com.saltmarket.shareboard.model.vo.*, com.saltmarket.common.model.vo.*" %>
    <%
		Shareboard sb = (Shareboard)request.getAttribute("sb");
	...
	--%>    
        ...
        <div id="center">
            <div id="centertitle"><a href="<%= contextPath %>/list.sbo?currentPage=1&regionNo=0&status=A" 
                ...
                <h5 id="categorytitle" class=".inline"> - <%= sb.getRegionName() %></h5>
            </div>

            <div class="black boardtitle" style="font-size: 20px;">
                <div>
                	<% if(sb.getShareStatus().equals("Y")) { %>
                		...
                		<br><hr>
                	<% } %>
                	No.<%= sb.getShareboardNo() %> : <%= sb.getBoardTitle() %>
                </div>
            </div>

            <div id="board-info">
                <a href="" class="black underline-none"><i class="fas fa-user" style="color: #000000;"></i>
                    <%= sb.getUserName() %>
                </a>
                <i class="far fa-comment-dots" style="color: #000000;"></i>
                	<span id="replycount"><%= sb.getReplyCount() %></span>
            ...
            <div id="board-bottom">
                ...
                <div id="update-delete">
                    ...
                    <% if(
                    		(loginUser != null) && (
                    				loginUser.getUserNo() == sb.getUserNo() ||
             						loginUser.getUserNo() == 1 ||
                    				loginUser.getUserNo() == 2)
                    		) { %>
	                    <button type="button" class="btn-margin btn btn-outline-primary btn-sm"
	                        onclick="location.href='<%= contextPath %>/manageUpdateForm.sbo?bno=<%= sb.getShareboardNo() %>'">수정</button>
						<button type="button" class="btn btn-outline-danger btn-sm"
						        onclick="showConfirmationModal(<%= sb.getShareboardNo() %>)">삭제</button>
					<% } %>
            ...
            <!-- 댓글 입력 폼 -->
			<div style="display: flex; align-items: center; background-color: #b3b3b3; padding: 15px;">
            			...
            			success : function(list) {
            				...
            				for(let i in list) {
            					...
            					result += "<div class='comment'>"
            								...
            								+ "<a href='' class='black underline-none'><span class='comment-author'>" + list[i].userName + "</span></a>"
