...
public class FreeboardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			// 게시글 작성 정보 뽑기
			String userName = multiRequest.getParameter("userNo");
			...
			// 서비스 요청
			int result = new FreeboardService().insertBoard(fb, imgList);
			
			// 결과 응답페이지
			if(result > 0) {
				request.getSession().setAttribute("alertMsg", "게시글 작성에 성공했습니다.");
				response.sendRedirect(request.getContextPath() + "/detail.fbo?bno=" + fb.getFreeboardNo());
			} else {
				if(imgList != null) {
