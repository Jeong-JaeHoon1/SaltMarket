...
<%@ include file="./boardManageSidebar.jsp"%>
    ...
    <div id="center">
        ...
        <div id="centertitle">나눔게시판 수정</div>

		<!-- 게시글 수정하기 입력받는 부분 -->
        <form action="<%= contextPath %>/manageUpdate.sbo" method="post" enctype="multipart/form-data">
        
        	<!-- 게시글 번호를 hidden 으로 넘기기 -->
			<input type="hidden" name="bno" value="<%= sb.getShareboardNo() %>">
                	...
                	지역 :
        				<select name="regionNo">
							<% for(Region r : regionList) { %>
								<option value="<%= r.getRegionNo() %>" <%= sb.getRegionNo() == r.getRegionNo() ? "selected" : "" %>>
									<%= r.getRegionName() %>
								</option>
							<% } %>
						</select>
					&nbsp;나눔여부 : <input type="checkbox" id="status" name="status" value="Y" style="width: 10px;" <% if(sb.getShareStatus().equals("Y")) { %> checked <% } %>><label for="status">&nbsp;완료</label>
                </h6>
                <!-- 제목 수정부분 -->
                <input type="text" name="title" size="" value="<%= sb.getBoardTitle() %>" placeholder="제목을 입력하세요" required>
            </div>

			<!-- 내용 수정부분 -->
            <textarea id="mytextarea" name="content" placeholder="내용을 입력하세요" required><%= sb.getBoardContent() %></textarea>
